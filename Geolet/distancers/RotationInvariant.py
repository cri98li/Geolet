...
class RotationInvariant_distancer(DistancerInterface):
    ...
    def _computeDist(self, tr_tid, tr_time, tr_X, geo_t, geo_X):
        ...
        for i, tid in enumerate(unique_tr):
            best_index[i], distances[i] = RotationInvariantBestFitting(tr_time[tr_tid == tid], tr_X[tr_tid == tid], geo_t, geo_X,
                                                        normalizer=self.normalizer)
        return best_index, distances

def RotationInvariantBestFitting(tr_time, tr_X, geo_time, geo_X, normalizer: NormalizerInterface, step=1):
    if len(geo_time) > len(tr_time):
        return RotationInvariantBestFitting(geo_time, geo_X, tr_time, tr_X, normalizer)
